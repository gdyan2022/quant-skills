...
## ⚠️ Plugin 根目录变量（执行脚本前确认已设置）

本 skill 的所有脚本引用都用 `$CLAUDE_PLUGIN_ROOT`（plugin 根）和 `$CLAUDE_PLUGIN_DATA`（持久化目录）定位。

- **Plugin 安装模式**：Claude Code 会自动注入这两个变量，直接用即可
- **手动 clone + symlink 模式**：两个变量为空，你需要从 skill 目录回退两级推导：

```bash
if [ -z "${CLAUDE_PLUGIN_ROOT:-}" ]; then
  # skill 目录 = <plugin_root>/skills/wind-db/，上两级就是 plugin 根
  export CLAUDE_PLUGIN_ROOT="$(cd ~/.claude/skills/wind-db/../.. 2>/dev/null && pwd)"
fi
```

下文命令里的 `$CLAUDE_PLUGIN_ROOT` 都假设已设置；脚本内部也有 fallback 逻辑，单独跑脚本不会炸。
