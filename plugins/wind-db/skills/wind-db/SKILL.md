...
## ⚠️ Plugin 根目录变量（执行脚本前必须先推导）

本 skill 的所有脚本引用都用 `$CLAUDE_PLUGIN_ROOT`（plugin 根）定位。**Claude Code 的 Bash 工具并不总是自动注入这个变量**——在 Bash 工具调用里 `$CLAUDE_PLUGIN_ROOT` 可能是空的，必须在执行任何脚本前先推导。

**推导逻辑（三层 fallback，和 `/wind-db:setup` 完全一致）：**

```bash
PLUGIN_ROOT=""

# 1. 环境变量（某些场景 Claude Code 会注入）
...
[ -z "$PLUGIN_ROOT" ] && { echo "✗ 找不到 wind-db plugin，请通过 /plugin install 安装" >&2; exit 1; }
export CLAUDE_PLUGIN_ROOT="$PLUGIN_ROOT"
```

推导好后再调 `bash "$CLAUDE_PLUGIN_ROOT/scripts/dict.sh" -t <表名>` 这类命令。**绝不**硬编码 `~/workspace/...` 或其他开发者路径。
...
凭据存放在 **plugin 持久化目录**（升级不会丢）。运行时脚本通过 `$CLAUDE_PLUGIN_ROOT`（代码目录）和 `$CLAUDE_PLUGIN_DATA`（持久化目录）定位文件，你**不需要手动计算路径**。
...
Claude 会通过对话依次问你数据库地址/端口/账密、字典站凭据等字段，确认后写入 `.env`，并可选注册 `dbhub-wind` MCP。已配置时会提示你传 `/wind-db:setup --reset` 重填。

也可以手动跑老的交互式 bash 向导（**只在终端有 tty 时**，Claude 会话内会卡住）：
```bash
...
cp .env.example "${CLAUDE_PLUGIN_DATA:-.}/.env"
vim "${CLAUDE_PLUGIN_DATA:-.}/.env"
...
```bash
ENV_FILE="${CLAUDE_PLUGIN_DATA:-$CLAUDE_PLUGIN_ROOT}/.env"
...
```bash
bash $CLAUDE_PLUGIN_ROOT/scripts/dict.sh -t AShareIncome   # 按英文表名
bash $CLAUDE_PLUGIN_ROOT/scripts/dict.sh -t 利润表          # 按中文名
```
...
```bash
bash $CLAUDE_PLUGIN_ROOT/scripts/dict.sh STATEMENT_TYPE    # 字段名
bash $CLAUDE_PLUGIN_ROOT/scripts/dict.sh 合并报表          # 业务概念，想看 FAQ 交叉讨论
```
...
```bash
bash $CLAUDE_PLUGIN_ROOT/scripts/dict.sh -l                    # 全量 TSV，约 315 KB
bash $CLAUDE_PLUGIN_ROOT/scripts/dict.sh -l | grep -F 利润表    # 按中文名管道过滤
bash $CLAUDE_PLUGIN_ROOT/scripts/dict.sh -l | grep -v '\[PIT\]' # 过滤掉 PIT 版本
bash $CLAUDE_PLUGIN_ROOT/scripts/dict.sh -l | awk -F'\t' '$3=="[STD]" && $4~/^中国A股/'   # A 股的标准表
```
