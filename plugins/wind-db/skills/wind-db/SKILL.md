...
**⚠️ 不要依赖 dbhub 返回的字段 comment 来推理字段含义**。dbhub 的 `search_objects` / schema 查询会返回字段名、类型、以及数据库本身的 `COMMENT`——这些 comment 往往：

- **残缺**：大量字段没有 comment（Wind 的 Oracle/MSSQL 实例里很多字段 comment 是空的）
...
### 4.1.5 告诉用户字段含义时必须标注来源

当你回复字段含义、值编码、业务规则时，**必须在括号里标注信息来源**，让用户知道这条结论的权威性。格式：

- `(来源：数据字典)` 或简写 `(字典)` —— 权威
- `(来源：数据库 comment)` 或简写 `(DB comment)` —— 线索级，用户需要自行验证
- `(综合：字典 + FAQ)` —— 来自字典的"常见问题"段
- `(推断)` —— 没有明确出处，Claude 自己根据字段名推理 —— **最弱的可靠性，尽量避免**
...
> `XXX_YYY_ZZZ` **(推断)** 根据字段名推测是 XXX 的 YYY，建议你用 `dict.sh <字段名>` 或联系 Wind 确认。

这个标注习惯保护用户不会把 Claude 的猜测当成权威结论。
...
| "字段**中文含义 / 值编码 / 业务主键**" | `dict.sh <字段>` 或 `dict.sh -t <表>` | ~~dbhub schema comment 只是线索~~ |
